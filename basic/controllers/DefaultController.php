...
class DefaultController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (!$this->StartBackup()) {
            //render error
            $flashError = 'error';
            $flashMsg = 'Error al crear el archivo';
        ...
        $flashError = 'success';
        $flashMsg = 'El archivo ha sido creado !!!';
    ...
    public function actionDelete($file) {
        ...
        if (isset($file)) {
            ...
            if (file_exists($sqlFile)) {
                unlink($sqlFile);
                $flashError = 'success';
                $flashMsg = 'The file ' . $sqlFile . ' Ha sido eliminado correctamente.';
            } else {
                $flashError = 'error';
                $flashMsg = 'El archivo ' . $sqlFile . ' no ha sido encontrado.';
            }
        } else {
            $flashError = 'error';
            $flashMsg = 'El archivo ' . $sqlFile . ' no ha sido encontrado.';
    ...
    public function actionIndex() {
        ...
        if ($list_files) {
            ...
            foreach ($list as $id => $filename) {
                ...
                $columns['Tamano'] = filesize($path . $filename);

                $columns['Fecha_creacion'] = date('Y-m-d H:i:s', filectime($path . $filename));
                $columns['Fecha_modificacion'] = date('Y-m-d H:i:s', filemtime($path . $filename));
    ...
    public function actionRestore($filename) {
        ...
        if (isset($filename)) {
            $sqlFile = $this->path . basename($filename);
            $flashError = 'success';
            $flashMsg = 'Copia restaurada Correctamente :)';
        } else {
            $flashError = 'error';
            $flashMsg = 'Parece que hay problemas con el nombre del Fichero.';
        }
    ...
    protected function updateMenuItems($model = null) {
        ...
        switch ($this->action->id) {
            ...
            case 'create': {
                    $this->menu[] = array('label' => Yii::t('app', 'Mostrar Copias de Seguridad'), 'url' => array('index'));
                }
                break;
            case 'upload': {
                    $this->menu[] = array('label' => Yii::t('app', 'Crear Copia de Seguridad'), 'url' => array('create'));
                }
                break;
            default: {
                    $this->menu[] = array('label' => Yii::t('app', 'Mostrar Copias de Seguridad'), 'url' => array('index'));
                    $this->menu[] = array('label' => Yii::t('app', 'Crear Copia de Seguridad'), 'url' => array('create'));
                    $this->menu[] = array('label' => Yii::t('app', 'Subir Copia de Seguridad'), 'url' => array('upload'));
                    $this->menu[] = array('label' => Yii::t('app', 'Inicio'), 'url' => Yii::$app->HomeUrl);
                }
                break;
        }
