...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`actividades` (
  ...
  `detalles_celebracion` TEXT NULL COMMENT 'Detalles de celebración de la actividad si es necesario o NULL si no hay.',
  -- -- Lugar -- --
  -- NO -- `lugar_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Lugar de celebración de la actividad o CERO si no existe o aún no está indicado (como si fuera NULL).',
  `direccion` TEXT NULL COMMENT 'Dirección concreta del lugar de celebración de la actividad o NULL si no se conoce, aunque no debería estar vacío este dato.',
  `como_llegar` TEXT NULL COMMENT 'Notas sobre cómo llegar a la dirección indicada o NULL si no hay indicaciones de como llegar.',
  `notas_lugar` TEXT NULL COMMENT 'Notas adicionales sobre el lugar de celebración de la actividad que no forman parte de la dirección o de las indicaciones, o NULL si no hay.',
  `area_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Area/Zona de celebración de la actividad o CERO si no existe o aún no está indicado (como si fuera NULL).',
  `notas` TEXT NULL COMMENT 'Notas adicionales sobre la actividad que no forman parte de las posibles notas anteriores o NULL si no hay.',
  `url` TEXT NULL COMMENT 'Dirección web externa (opcional) que enlaza con la página "oficial" de la actividad o NULL si no hay.',
  ...
  -- -- Imágenes adicionales: Tabla de Relación  -- --
  ...
  -- SI -- Etiquetas: Tabla de Relación -- --
  -- NO --`etiquetas_id` TEXT NULL COMMENT 'Lista de identificadores de "etiquetas" con los que se relaciona la actividad o NULL si no hay. La ultima etiqueta de la lista debe llevar el separador también.',
  -- NO --`etiquetas_nombre` TEXT NULL COMMENT 'Lista de datos "nombre" de las propias "etiquetas" con las que se relaciona la actividad o NULL si no hay. La ultima etiqueta de la lista debe llevar el separador también.',
  -- -- Clasificación: Tabla de Relación -- --
  ...
  `fecha_terminacion` DATETIME NULL DEFAULT NULL COMMENT 'Fecha y Hora de terminación de la actividad. Debería estar a NULL si no está terminada.',
  `notas_terminacion` TEXT NULL COMMENT 'Notas visibles sobre el motivo de la terminación de la actividad.',
  -- -- Registro de Denuncia+Bloqueo -- --
  ...
  `notas_bloqueo` TEXT NULL COMMENT 'Notas visibles sobre el motivo del bloqueo de la actividad o NULL si no hay -se muestra por defecto según indique "bloqueada"-.',
  -- -- Patrocinios: Tabla de Relación -- --
  -- -- Seguimientos: Tabla de Relación -- --
  -- -- Participación -- --
  ...
  -- -- Formulario de Participación: Tabla de Relación? -- --
  -- -- Participantes: Tabla de Relación -- --
  ...
  `crea_fecha` DATETIME NULL COMMENT 'Fecha y Hora de creación de la actividad o NULL si no se conoce por algún motivo.',
  ...
  `modi_fecha` DATETIME NULL COMMENT 'Fecha y Hora de la última modificación de la actividad o NULL si no se conoce por algún motivo.',
...
-- -----------------------------------------------------
-- Actividades_Etiquetas: actividad_id, etiqueta_id
...
-- -----------------------------------------------------
-- Clasificación: id, nombre
DROP TABLE IF EXISTS `daw2_actividades`.`clasificaciones` ;
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`clasificaciones` (
  `id` INT(12) UNSIGNED NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(25) NULL,
  `descripcion` TEXT NULL COMMENT 'Texto adicional que describe la categoria o clasificación para las etiquetas que engloba.',
...
-- -----------------------------------------------------
-- Clasificación_Etiquetas: id, clasificacion_id, etiqueta_id, padre_id
...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`clasificacion_etiquetas` (
  ...
  `etiqueta_id` INT(12) UNSIGNED NOT NULL COMMENT 'Etiqueta relacionada.',
  `clasificacion_etiqueta_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Clasificacion_Etiqueta relacionada. Nodo padre de la jerarquia o CERO si es nodo raiz.',
...
-- -----------------------------------------------------
-- Patrocinios: id, actividad_id, anuncio_id --
...
-- -----------------------------------------------------
-- Seguimientos: id, actividad_id, usuario_id, fecha_seguimiento
...
-- -----------------------------------------------------
-- Participación: id, actividad_id, usuario_id, fecha_registro, [datos_participacion]
--    -- -- Datos de Participación: Tabla de Relación o bloque de texto -- -- 
...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`actividad_participantes` (
  ...
  `fecha_registro` DATETIME NOT NULL COMMENT 'Fecha y Hora de activación del seguimiento de la actividad por parte del usuario.',
...
-- -----------------------------------------------------
-- Comentarios: id, actividad_id, texto, Registro de Usuario, padre_id, cerrado, Registro de Denuncia+Bloqueo
...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`actividad_comentarios` (
  ...
  `crea_fecha` DATETIME NULL COMMENT 'Fecha y Hora de creación del comentario o NULL si no se conoce por algún motivo.',
  ...
  `modi_fecha` DATETIME NULL COMMENT 'Fecha y Hora de la última modificación del comentario o NULL si no se conoce por algún motivo.',
  `notas_admin` TEXT NULL COMMENT 'Notas adicionales para los administradores sobre la actividad o NULL si no hay.',
  -- -- Comentario y relacion con posible Padre -- --
  `texto` TEXT NOT NULL COMMENT 'El texto del comentario.',
  `comentario_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Comentario relacionado, si se permiten encadenar respuestas. Nodo padre de la jerarquia de comentarios, CERO si es nodo raiz.',
  ...
  `fecha_bloqueo` DATETIME NULL COMMENT 'Fecha y Hora del bloqueo del comentario. Debería estar a NULL si no está bloqueado o si se desbloquea.',
  `notas_bloqueo` TEXT NULL COMMENT 'Notas visibles sobre el motivo del bloqueo del comentario o NULL si no hay -se muestra por defecto según indique "bloqueado"-.',
...
-- -----------------------------------------------------
-- Usuario: email, password, nick, nombre, apellidos, fecha_nacimiento, dirección (opcional), Área de localización (se recomienda obligatoria), rol (N:Normal, M:Moderador, P:Patrocinador, A:Administrador), Opciones de Aviso, fecha/hora de registro, registro confirmado, bloqueado, ultimo acceso, intentos acceso, [Valoración usuario,] Áreas de moderación, Seguimientos, Participación, Comentarios, Avisos.
...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`usuarios` (
  ...
  `rol` CHAR(1) NOT NULL COMMENT 'Tipo de Perfil: N=Normal, M=Moderador, P=Patrocinador, A=Administrador',
  -- Opciones de Aviso --
  ...
  `avisos_marcar_borrados` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Indicador de marcar los correos leidos como borrados después de un tiempo  0:No marcar, >0:tiempo en días',
  ...
  -- Valoracion Usuario -- *** NO se implementa por ahora
  -- Areas de Moderacion: Tabla de Relación --
  -- Seguimientos: Tabla de Relación ya comentada--
  -- Participación: Tabla de Relación ya comentada--
  -- Comentarios: Tabla de Relación ya comentada--
  -- Avisos: Tabla de Relación--
...
-- -----------------------------------------------------
-- Table `daw2_actividades`.`avisos`
-- -----------------------------------------------------
-- Avisos: fecha/hora, código de clase de aviso (A:Aviso, N:Notificación, D:Denuncia, C:Consulta), texto, Usuario destino (NULO si es para un moderador o administrador y no se conoce su usuario o aún no ha sido aceptado el aviso por uno de ellos), Usuario origen (NULO si es generado por el sistema), Actividad (NULO si no está relacionado con una actividad), Comentario (NULO si no está relacionado con un comentario), fecha/hora de lectura (NULO si no se ha leído o se ha desmarcado como leído), fecha/hora de borrado (NULO si no se ha borrado), fecha/hora de aceptación (NULO si no se ha aceptado para su gestión por un moderador o administrador, no usado en otros usuarios).
...
CREATE TABLE IF NOT EXISTS `daw2_actividades`.`usuario_avisos` (
  ...
  `clase` CHAR(1) NOT NULL DEFAULT 'M' COMMENT 'código de clase de aviso: A=Aviso, N=Notificación, D=Denuncia, C=Consulta, M=Mensaje Genérico,...',
  `texto` TEXT NULL COMMENT 'Texto con el mensaje de aviso.',
  `destino_usuario_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Usuario relacionado, destinatario del aviso, o NULL si no es para administración y no está gestionado.',
  `origen_usuario_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Usuario relacionado, origen del aviso, o NULL si es del sistema.',
  `actividad_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Actividad relacionada o NULL si no tiene que ver con una actividad.',
  `comentario_id` INT(12) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Comentario relacionado o NULL si no tiene que ver con un comentario.',
  `fecha_lectura` DATETIME NULL COMMENT 'Fecha y Hora de lectura del aviso o NULL si no se ha leido o se ha desmarcado como tal.',
  `fecha_borrado` DATETIME NULL COMMENT 'Fecha y Hora de la marca de borrado del aviso o NULL si no se ha marcado como borrado.',
  `fecha_aceptado` DATETIME NULL COMMENT 'Fecha y Hora de aceptación del aviso o NULL si no se ha aceptado para su gestión por un moderador o administrador. No se usa en otros usuarios.',
